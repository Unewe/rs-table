...
type VerticalScrollbarProps = { headerHeight: number, containerRef: React.RefObject<HTMLDivElement> };

export const VerticalScrollbar: React.FC<VerticalScrollbarProps> = React.memo(({headerHeight, containerRef}) => {

  const onDrag = (event: PointerEvent) => {
    event.stopPropagation();
    scrollbarRef.current?.classList.add("rs-active");
    scrollBlocker = true;
    const position = parseInt(verticalScrollBarRef.current!.style.top) + event.movementY;
    const maxHeight = (containerRef.current!.clientHeight - 30);
    verticalScrollBarRef.current!.style.top = `${position < 0 ? 0 : position > maxHeight ? maxHeight : position}px`;

    const scrollPosition = position / maxHeight * containerRef.current!.scrollHeight;
  ...
  const scrollTo = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    const scrollPosition = (y - top) / containerRef.current!.clientHeight * containerRef.current!.scrollHeight;
    containerRef.current?.scrollTo({top: scrollPosition});
  ...
  return (
    ...
    onPointerDown={scrollTo}>
      <Draggable onDrag={onDrag} onDrop={onDrop} immediate={true}>
        <div className={"rs-scrollbar-thumb"} ref={verticalScrollBarRef} style={{top: "0px"}}/>
      </Draggable>
    </div>
  );
});
